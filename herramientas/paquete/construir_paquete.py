...
PACKAGE = ROOT / 'producto/paquete_fundacional/v0.1'
...
COMPONENTS = [
    ...
    ('Operación conversacional', '0.1', 'producto/metodo/operacion_conversacional/v0.1'),
    ('Paquete fundacional', '0.1', 'producto/paquete_fundacional/v0.1'),
]
TOOL_FILES = [
...
def main():
    ...
    for label, version, directory in COMPONENTS:
        ...
        for p in sorted(base.rglob('*')):
            if p.is_file():
                if p.is_symlink() or p.suffix not in {'.md', '.mjs', '.xlsx', '.docx', '.json'}:
    ...
    for name in TOOL_FILES:
        ...
        paths[name] = ('Herramientas documentales', 'conjunto 0.1')
    for p in [INVENTORY, MANIFEST]:
        paths[p.relative_to(ROOT).as_posix()] = ('Paquete fundacional', '0.1')
    lines = ['# Inventario de la distribución', '', 'Conjunto fundacional 0.1 · 11 de septiembre de 2026', '',
             ...
             'Las versiones base se fijan con la revisión de integración del conjunto 0.1. El manifiesto distingue el contenido exacto tras las correcciones editoriales. Las vistas derivadas y herramientas están identificadas como tales.', '']
    ...
    for name, data in sorted(content.items()):
        group, version = paths.get(name, ('Entrada de distribución', '0.1'))
    ...
    manifest = {'producto': 'Go2Rev', 'version_conjunto': '0.1', 'fecha': '2026-09-11',
                'estado': 'Construcción cerrada; base teórica sin pruebas de la metodología',
                'revision_integracion': 'FND08 2026-09-11',
    ...
    dest = output / 'Go2Rev_fundacional_v0.1.zip'
    ...
    (output / 'Go2Rev_fundacional_v0.1.sha256').write_text(sha(dest.read_bytes()) + '  ' + dest.name + '\n', encoding='utf-8')
