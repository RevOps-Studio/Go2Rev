...
COMPONENTS = [
    ...
    ('Operación conversacional', '0.4', 'producto/metodo/operacion_conversacional/v0.4'),
    ('Definición de formatos', '0.1', 'producto/metodo/esquema/v0.1'),
    ('Paquete fundacional', '0.6', 'producto/paquete_fundacional/v0.6'),
]
TOOL_FILES = [
...
def main():
    ...
    for name in TOOL_FILES:
        ...
        paths[name] = ('Herramientas documentales', 'conjunto 0.6')
    for p in [INVENTORY, MANIFEST]:
        paths[p.relative_to(ROOT).as_posix()] = ('Paquete fundacional', '0.6')
    lines = ['# Inventario de la distribución', '', 'Conjunto fundacional 0.6 · 17 de septiembre de 2026', '',
    ...
    for name, data in sorted(content.items()):
        group, version = paths.get(name, ('Entrada de distribución', '0.6'))
    ...
    manifest = {'producto': 'Go2Rev', 'version_conjunto': '0.6', 'fecha': '2026-09-17',
                'estado': 'Base teórica sin validación sustantiva; activación y enrutado revisados tras un intento de arranque',
                'revision_integracion': 'Identidad y enrutado metodológico 2026-09-17',
    ...
    dest = output / 'Go2Rev_fundacional_v0.6.zip'
    ...
    (output / 'Go2Rev_fundacional_v0.6.sha256').write_text(sha(dest.read_bytes()) + '  ' + dest.name + '\n', encoding='utf-8', newline='\n')
